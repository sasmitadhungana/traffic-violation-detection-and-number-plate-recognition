...
VIDEO_FILE = os.path.join(BASE_DIR, "videos", "Testvideoo.mp4")
...
# Item #1 (FPS optimization): run YOLO inference on a smaller, fixed image
...
# Item #1: the traffic light itself does not need to be re-detected every
...
# Item #1: set to "cuda" if you have a working GPU + matching PyTorch/CUDA
# build (biggest possible speed win - often 5-10x). Leave "cpu" otherwise.
INFERENCE_DEVICE = "cpu"
# Item #1: half-precision (FP16) inference - only takes effect on CUDA;
...
class TrafficViolationDetector:
    def __init__(self):
        ...
        # Item #2 safeguard: records whether each vehicle was first seen
        # BEFORE or AFTER the stop line. A vehicle first spotted already
        # past the line (already mid-crossing, or driving away from the
        # camera) is never blamed for a violation - we only judge vehicles
        # we actually watched approach the line from before it.
        ...
        # Item #1 (FPS): cache used when skipping traffic-light inference
        # on some frames (see TRAFFIC_LIGHT_SKIP_FRAMES).
        self._cached_traffic_state = "UNKNOWN"
    ...
    def _detect_traffic_light(self, frame):
        ...
        # Item #1 (FPS): only re-run the traffic light model every
        # TRAFFIC_LIGHT_SKIP_FRAMES frames; on skipped frames, reuse the
        # last detected candidates/state (signals don't change that fast)
        # and still feed the same cached state into the vote history so
        # RED_CONFIRM_FRAMES/STATE_VOTE_MIN behave the same either way.
        if TRAFFIC_LIGHT_SKIP_FRAMES > 1 and self.frame_count % TRAFFIC_LIGHT_SKIP_FRAMES != 0:
            cached_state = self._cached_traffic_state
            self.state_history.append(cached_state)
            if len(self.state_history) > STATE_HISTORY_LEN:
                self.state_history.pop(0)
            if cached_state == "RED":
                self.red_frames += 1
            return self._cached_traffic_candidates
        ...
        # Item #1: cache this frame's real result so skipped frames can
        # reuse it instead of re-running inference.
        self._cached_traffic_state = current_state
    ...
    def _detect_vehicles(self, frame):
        ...
        try:
            ...
            # Item #1: smaller imgsz + device/half for faster inference.
    ...
    def _save_violation_citation(self, output_frame, vehicle_crop, plate_image,
                                  vehicle_id, plate_text, video_timestamp):
        """
        Item #4: builds a single enforcement-style evidence image:
        the full annotated scene (stop line + red light + boxes already
        drawn) on top, with the vehicle crop and plate crop inset at the
        bottom, plus a header banner with violation number, vehicle ID,
        plate text, and timestamp - saved as Violation_001.jpg, etc.
        """
        try:
            ...
            # Header banner
            banner_h = 70
            citation = np.zeros((h + banner_h + 120, w, 3), dtype=np.uint8)
            citation[:] = (20, 20, 20)
            ...
            # Inset strip: vehicle crop + plate crop side by side
            ...
            _paste_inset(vehicle_crop, "VEHICLE")
    ...
    def _check_violation(self, vehicle, frame, output_frame):
        ...
        reduction). A violation requires ALL of:
            ...
            2. Vehicle was watched approaching from BEFORE the stop line,
               not first spotted already past it (item #2 safeguard)
            3. Vehicle then crosses the line (fix #4)
            4. Vehicle is moving forward, not jitter/reversing (fix #11)
            5. Vehicle has not already been recorded (dedupe, fix #4 orig)
            6. Vehicle isn't a parked/stationary false detection (item #3)
        """
        vehicle_id = vehicle['id']
        ...
        # Item #2: never blame a vehicle we first detected already past
        # the line (already mid-crossing, or driving away) - we can't be
        # sure of its history before it entered view.
        ...
        # Item #4: build a single "citation style" evidence image that
        # combines the full scene (with stop line + red light already
        # drawn in output_frame), the cropped vehicle, the cropped plate,
        # and a timestamp/details header - the kind of single image a
        # traffic enforcement system would file per violation.
        violation_file = self._save_violation_citation(
            output_frame, vehicle_crop if vehicle_crop.size > 0 else None,
            best_plate_image, vehicle_id, plate_text, video_timestamp
        )
    ...
    def _draw_annotations(self, frame, lights, vehicles):
        ...
        for vehicle in vehicles:
            ...
            # Item #7: two-line "ID:34 / Plate: BA2CHA5678" label for a
            # clearer demo, instead of squeezing both onto one line.
            if plate_text:
    ...
    def _save_report(self):
        ...
        for path in (report_file, timestamped_file):
            with open(path, 'w', newline='') as f:
                ...
                writer.writerow([
                    "Vehicle ID", "Plate", "Frame", "Time", "Signal", "Status",
    ...
    def process_video(self):
        ...
        while True:
            ...
            for v in vehicles:
                ...
                # Item #2 safeguard: remember which side of the line this
                # vehicle was on the FIRST time we ever saw it tracked.
                if vid not in self.vehicle_first_seen_side:
                    dist = self._stop_line_signed_distance(v['center_x'], v['bottom_y'])
        ...
        # Item #8: final summary in the exact requested format
        print("\n" + "=" * 32)
        print("Video Finished")
        print(f"Vehicles Detected : {len(self.vehicle_history)}")
        print(f"Violations : {self.total_violations}")
        print(f"Red Frames : {self.red_frames}")
