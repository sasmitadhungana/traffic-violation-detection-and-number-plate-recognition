...
================================================================
A computer vision system that detects red light violations and captures
license plates from surveillance video.
...
class DetectionConfig:
    ...
    VEHICLE_CONF = 0.30
    PLATE_CONF = 0.15  # Lowered to catch more plates
    
    # Model settings
    DETECTION_IMGSZ = 800  # Increased for better detail on small plates
...
class LicensePlateOCR:
    """
    OCR for license plate reading using PaddleOCR.
    ...
    def preprocess_plate(self, image: np.ndarray) -> List[Tuple[str, np.ndarray]]:
        ...
        if h < 80 or w < 150:
            ...
            # Upscaled + CLAHE
            upscaled_clahe = clahe.apply(upscaled)
            processed.append(("upscaled_clahe", upscaled_clahe))
...
"""
    ...
    def detect_license_plate(self, frame: np.ndarray, vehicle_box: Tuple[int, int, int, int]) -> Optional[Dict]:
        ...
        Uses multi-scale detection with larger padding for better results.
        """
        ...
        # Increased padding to capture more area around the plate
        padding = 60
    ...
    def _get_best_plate(self, vehicle_id: int, fallback_frame: np.ndarray,
        ...
        Tries multiple frames and upscales plates for better OCR.
        """
        ...
        for entry in frames:
            ...
            # Upscale the plate image if it's too small
            plate_image = plate['image']
            h, w = plate_image.shape[:2]
            if w < 100 or h < 30:
                scale = max(3.0, 300.0 / w)
                new_w = int(w * scale)
                new_h = int(h * scale)
                plate_image = cv2.resize(plate_image, (new_w, new_h), cv2.INTER_CUBIC)
            
            # Read plate text with upscaled image
    ...
    def _generate_citation(self, output_frame: np.ndarray, vehicle_crop: np.ndarray,
        ...
        """
        Generate a citation image showing the violation evidence.
        """
        try:
    ...
    def process_video(self):
        ...
        while True:
            ...
            # Cleanup - remove frame buffer for violated vehicles
            ...
            if VideoConfig.SHOW_PREVIEW:
                ...
                if key == ord('q'):
                    ...
                    print(f"{lane['name']} line: {lane['stop_line_y']}")
                elif key == ord('['):
                    lane = self.lanes[self.active_lane_index]
